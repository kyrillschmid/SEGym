...
TASK_TYPES = {
    "generate_code_snippets_for_changes": [
        "generate code based on an issue description to be implemented in a repo. Add line numbers in the code!",
        "Code snippets for changes",
    ],
    "create_patch_string": [
        "create patch strings for a given Python repository. Make sure to consider the line numbers in the patch!",
        "Patch string",
    ],
}
...
class Solver:
    ...
    def generate_patch(
        self, iteration, issue_description, api, model, last_patch=None, feedback=None
    ):
        ...
        if last_patch is not None and feedback is not None:
            ...
            user_prompt += (
                f"""Feedback for last patch from execution environment:\n{feedback}"""
            )
        ...
        for i, task_type in enumerate(TASK_TYPES.keys()):
            ...
            user_prompt += (
                f"Here is the output from a previous task that might be useful:\n"
            )
            ...
            for f in files:
                ...
                if affected_files and f in affected_files:
                    self.print_file_contents(
                        os.path.join(root, f), file, indent=subindent + " " * 4
                    )
